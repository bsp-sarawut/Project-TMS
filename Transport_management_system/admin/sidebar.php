    <!-- sidebar.php -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">

    <div class="sidebar" id="sidebar">
        <button class="close-btn" id="close-btn">×</button>
        <h3 class="text-center text-white mb-3">Dashboard</h3>
        <a href="admin_dashboard.php" class="active"><i class="fas fa-home"></i> หน้าแรก</a>
        <a href="route.php"><i class="fas fa-map"></i> จัดการข้อมูลเส้นทาง</a>
        <a href="car.php"><i class="fas fa-car"></i> จัดการข้อมูลรถ</a>
        <a href="driver.php"><i class="fas fa-user-tie"></i> จัดการข้อมูลคนขับ</a>
        <a href="choice.php"><i class="fas fa-calendar-alt"></i> จัดตารางลงทะเบียน</a>
        <a href="enrollment.php"><i class="fas fa-file-alt"></i> ข้อมูลการลงทะเบียน</a>
        <a href="queue.php"><i class="fas fa-bus"></i> จัดตารางรถ</a>
        <a href="show_queue.php"><i class="fas fa-table"></i> ตารางรถ</a>
        <a href="queue_log.php"><i class="fas fa-road"></i> ข้อมูลการเดินทาง</a>
        <a href="#"><i class="fas fa-bell"></i> แจ้งข่าวสาร</a>
        <a href="#" class="logout" id="logout"><i class="fas fa-sign-out-alt"></i> ออกจากระบบ</a>
    </div>