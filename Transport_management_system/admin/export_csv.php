...
// ฟังก์ชันสำหรับ export CSV ทั้งหมด
...
try {
    ...
    $params = [];
    if (!empty($_SESSION['export_search'])) {
        ...
        $params[':search'] = "%{$_SESSION['export_search']}%";
    }
    if (!empty($_SESSION['export_filter_date'])) {
        $sql .= " AND q.queue_date = :filter_date";
        $params[':filter_date'] = $_SESSION['export_filter_date'];
    }
    if (!empty($_SESSION['export_filter_province'])) {
        $sql .= " AND p.PROVINCE_ID = :filter_province";
        $params[':filter_province'] = $_SESSION['export_filter_province'];
    }
    if (!empty($_SESSION['export_filter_amphur'])) {
        $sql .= " AND a.AMPHUR_ID = :filter_amphur";
        $params[':filter_amphur'] = $_SESSION['export_filter_amphur'];
    }
